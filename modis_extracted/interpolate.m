...
for counter=1:length(AODFolderList)
    ...
    try 
        ...
        [xq yq] = meshgrid(41.3 + deltaY/2:0.1349:43.10889, -72.1880+deltaX/2:0.179562:-70.550);
        [row column]=size(xq);

        interpolatedData = zeros(row,column,40);
        ...
        disp(['interpolated_' AODFolderName '.mat']);
        save(['interpolated_' AODFolderName '.mat'],'data');
        cd(curdir);
    catch
        cd(curdir);
    end
