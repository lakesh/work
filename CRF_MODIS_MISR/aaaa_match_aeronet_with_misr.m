function [matches, matches_entries] = aaaa_match_aeronet_with_misr()

% matches have the following format:
% [AERONET file, AERONET entry, MISR file, MISR entry]
% 
% matches_entries have the following format:
% [AERONET entry (40 columns), MISR entry (20 columns)]
...
matches_entries = zeros(expected_matches_size, 25);
% matches_entries = zeros(expected_matches_size, 60);
...
for aeronet_site_index = 1 : length(aeronet_files)
    ...
    load('Satellite_MISR_data_raw_processed\aaa_per_location_mat_file\_fields_and_locs.mat', 'locations');
    ...
    for loc = 1 : size(locations, 1)
        ...
        % MODIS
        load(['Satellite_MISR_data_raw_processed\aaa_per_location_mat_file\file_num', num2str(loc), '.mat'], 'MISR_mat');
        ...
        for i = 1 : size(aeronet_data_better, 1)
            ...
            for j = match_found_at : size(MISR_mat, 1)
                current_misr = MISR_mat(j, :);
                
                if (areCloseInTime(current_aeronet(1 : 5), current_misr(1 : 5), THRESHOLD_TIME))
                    ...
                    if (found_matches > length(matches))
                        ...
                        matches_entries = [matches_entries; zeros(expected_matches_size, 25)];
%                         matches_entries = [matches_entries; zeros(expected_matches_size, 60)];
                    end;
                    
                    matches(found_matches, :) = [aeronet_site_index, i, loc, j];
                    matches_entries(found_matches, :) = [current_misr, current_aeronet([35, 31, 25, 11, 9])];
%                     matches_entries(found_matches, :) = [current_aeronet, current_misr];
                    match_found_at = j;
                    break;
                elseif (isSecondTimeInFuture(current_aeronet(1 : 3), current_misr(1 : 3)))
                    break;
                end;
            end;
        end;
