...
% separately MODIS, MISR and AERONET data, together with some other info
...
for i = 1 : length(locs_common)
    ...
    % set all NaNs to 0
    location_details(i).measurements_modis(isnan(measurements_modis)) = 0;
    location_details(i).measurements_misr(isnan(measurements_misr)) = 0;
    location_details(i).measurements_aeronet(isnan(measurements_aeronet)) = 0;
    
    % number of non-zero points
    location_details(i).count_measurements_modis = full(sum(location_details(i).measurements_modis ~= 0));
    location_details(i).count_measurements_misr = full(sum(location_details(i).measurements_misr ~= 0));
    location_details(i).count_measurements_aeronet = full(sum(location_details(i).measurements_aeronet ~= 0));
end;
