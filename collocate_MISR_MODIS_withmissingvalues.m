...
for i=1:row
    ...
    if matchFound == false
        ...
        %%%% Regard the MODIS as missing %%%%%%%%%% 
        %MODIS AOD value
        collocatedData(index,3) = 0;

        %IModis(xi)
        collocatedData(index,4) = 0;

        %AERONET AOD value
        collocatedData(index,5) = MISRdataPoint(1,2);
        %Latitude
        collocatedData(index,6) = MISRdataPoint(1,4);
        %Longitude
        collocatedData(index,7) = MISRdataPoint(1,5);
        %Day of year
        collocatedData(index,8) = MISRdataPoint(1,6);
        %Row
        collocatedData(index,9) = MISRdataPoint(1,9);
        %Column
        collocatedData(index,10) = MISRdataPoint(1,10);
        index = index + 1;
                
    end
end

%Populate the remaining MODIS collocated data with MISR as missing
%attribute
count = 0;
for i=1:row1
    MODISdataPoint = MODIS(i,:);
    
    if find(collocatedData(:,8) == MODISdataPoint(:,6) & collocatedData(:,9) == MODISdataPoint(:,9) & collocatedData(:,10) == MODISdataPoint(:,10))
       % do nothing
       count = count + 1;
    else
        %MISR AOD value as missing attribute
        collocatedData(index,1) = 0;

        %IMisr(xi)
        collocatedData(index,2) = 0;
